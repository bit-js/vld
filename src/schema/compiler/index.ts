...
export function compileArray(schema: ArraySchema, optional: boolean): any {
    ...
    if (typeof items === 'undefined') {
        ...
        const ctx = new ObjectCompileContext(['Array.isArray(o)']);
    ...
    if (typeof prefixItems === 'undefined')
        return Function('f', 'return (o)=>Array.isArray(o)&&o.every(f);')(checkItem);
...
export function compileNumberLiteral(val: string, checkInteger: boolean, schema: NumericSchema, optional: boolean): any {
    ...
    return `${optional ? `typeof ${val}==='undefined'||` : ''}Number.is${checkInteger ? 'isInteger' : 'isFinite'}(${val})${typeof schema.maximum === 'number' ? `&&${val}<=${schema.maximum}` : ''}${typeof schema.minimum === 'number' ? `&&${val}>=${schema.minimum}` : ''}${typeof schema.exclusiveMaximum === 'number' ? `&&${val}<${schema.exclusiveMaximum}` : ''}${typeof schema.exclusiveMinimum === 'number' ? `&&${val}>${schema.exclusiveMinimum}` : ''}${typeof schema.multipleOf === 'number' ? `&&${val}%${schema.multipleOf}===0` : ''}`;
}
...
/* eslint-disable */
export function compile(schema: any): (data: any) => boolean {
    if ('type' in schema) {
        const { type } = schema;
